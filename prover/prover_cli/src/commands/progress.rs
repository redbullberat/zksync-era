...
async fn get_one_batch_prgress(l1_batches_numberes: Vec<L1BatchNumber>) -> anyhow::Result<()> {
    ...
    let stats = conn
        .fri_prover_jobs_dal()
        .get_prover_jobs_stats_for_batch(l1_batches_numberes)
        .await;
...
pub(crate) async fn run(args: Args) -> anyhow::Result<()> {
    ...
    if let Some(l1_batch_number) = args.proof {
        get_one_batch_prgress(l1_batch_number).await
    } else {
